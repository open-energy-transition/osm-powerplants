...
from osm_powerplants.core import get_cache_dir, get_config
...
def get_osm_cache_paths(config: dict | None = None) -> tuple[str, str]:
    ...
    fn = actual_config.get("fn", "osm_data.csv")
    cache_dir = str(get_cache_dir(actual_config))
...
def determine_set_type(technology: str | None, config: dict[str, Any]) -> str | None:
    ...
    for set_type, technologies in set_mapping.items():
        if technologies and technology in technologies:
            return set_type
